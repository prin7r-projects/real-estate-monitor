...
export const getProfile: GetProfile<{ id: string }, Profile & { matches: (Match & { listing: Listing })[] }> = async (args, context) => {
  if (!context.user) {
    throw new HttpError(401, 'Not authenticated');
  }
...
export const createProfile: CreateProfile<{
  ...
  if (args.minPriceCents < 0 || args.maxPriceCents < 0) {
    throw new HttpError(400, 'Prices must be non-negative');
  }
  if (args.maxPriceCents > 0 && args.minPriceCents > args.maxPriceCents) {
    throw new HttpError(400, 'Min price must be less than max price');
  }
  if (args.minBedrooms < 0) {
    throw new HttpError(400, 'Bedrooms must be non-negative');
  }
  if (args.radiusKm <= 0 || args.radiusKm > 100) {
    throw new HttpError(400, 'Radius must be between 0 and 100 km');
  }
